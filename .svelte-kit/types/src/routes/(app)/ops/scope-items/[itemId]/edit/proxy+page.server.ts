...
export const load = async (event: Parameters<PageServerLoad>[0]) => {
	...
	// Convert service item data to form format
	const formData = {
	...
	const form = await superValidate(formData, zod4(serviceItemFormSchema), {
		errors: false
	});
...
export const actions = {
	default: async (event: import('./$types').RequestEvent) => {
		...
		try {
			...
			return fail(500, {
				form,
				message: 'Failed to update service item. Please try again.'
			});
		}
	}
};
