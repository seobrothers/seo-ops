<script lang="ts">
	...
	let editSalesRelatedDetails = $state(false);
	...
	const serviceDetailsSchema = z.object({
		...
		serviceScope: z.enum(['campaign', 'partner']),
	});
	const serviceDetailsForm = superForm({
		...
		serviceScope: data.serviceItem.serviceScope || 'campaign',
	...
	// Sales related details form setup
	const salesRelatedDetailsSchema = z.object({
		proposalMode: z.enum(['recurring', 'one_time', 'both', 'neither']),
		packageDisplay: z.string().nullable(),
		mcpDisplay: z.string().nullable(),
		recommendedPrice: z.number().nullable(),
		recommendedPriceCurrency: z.string().nullable(),
	});
	const salesRelatedDetailsForm = superForm({
		proposalMode: data.serviceItem.proposalMode || 'both',
		packageDisplay: data.serviceItem.packageDisplay,
		mcpDisplay: data.serviceItem.mcpDisplay,
		recommendedPrice: data.serviceItem.recommendedPriceCents ? data.serviceItem.recommendedPriceCents / 100 : null,
		recommendedPriceCurrency: data.serviceItem.recommendedPriceCurrency,
	}, { validators: zod4Client(salesRelatedDetailsSchema) });
	const { form: salesRelatedDetailsFormData } = salesRelatedDetailsForm;
	...
	const proposalModeOptions = [
		...
		{ value: 'both', label: 'Both One-Time and Recurring' },
		{ value: 'neither', label: 'Not Sellable' }
	];
	...
	function startEditingServiceDetails() {
		...
		$serviceDetailsFormData.serviceScope = data.serviceItem.serviceScope || 'campaign';
	...
	// Sales related details editing functions
	function startEditingSalesRelatedDetails() {
		// Reset form data to current values when starting to edit
		$salesRelatedDetailsFormData.proposalMode = data.serviceItem.proposalMode || 'both';
		$salesRelatedDetailsFormData.packageDisplay = data.serviceItem.packageDisplay;
		$salesRelatedDetailsFormData.mcpDisplay = data.serviceItem.mcpDisplay;
		$salesRelatedDetailsFormData.recommendedPrice = data.serviceItem.recommendedPriceCents ? data.serviceItem.recommendedPriceCents / 100 : null;
		$salesRelatedDetailsFormData.recommendedPriceCurrency = data.serviceItem.recommendedPriceCurrency;
		editSalesRelatedDetails = true;
	}

	function cancelEditingSalesRelatedDetails() {
		// Reset to original values
		$salesRelatedDetailsFormData.proposalMode = data.serviceItem.proposalMode || 'both';
		$salesRelatedDetailsFormData.packageDisplay = data.serviceItem.packageDisplay;
		$salesRelatedDetailsFormData.mcpDisplay = data.serviceItem.mcpDisplay;
	...
	// Format proposal mode for display
	function formatProposalMode(mode: string | null): string {
		if (!mode) return 'Not specified';
		const option = proposalModeOptions.find(opt => opt.value === mode);
	...
	// Format service scope for display
	function formatServiceScope(scope: string | null): string {
		if (!scope) return 'Campaign';
		const option = serviceScopeOptions.find(opt => opt.value === scope);
...
<div class="space-y-6 p-6">
	...
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<CardContent>
					{#if editServiceDetails}
						...
						>
							...
							<!-- Service Scope -->
							<div class="space-y-2">
								<label class="text-sm font-medium text-gray-700">Service Scope</label>
								<Select.Root type="single" bind:value={$serviceDetailsFormData.serviceScope}>
									<Select.Trigger>
										{$serviceDetailsFormData.serviceScope ?
											serviceScopeOptions.find(opt => opt.value === $serviceDetailsFormData.serviceScope)?.label || $serviceDetailsFormData.serviceScope
											: 'Select service scope...'}
									</Select.Trigger>
									<Select.Content>
										{#each serviceScopeOptions as option}
											<Select.Item value={option.value}>{option.label}</Select.Item>
										{/each}
									</Select.Content>
								</Select.Root>
								<input type="hidden" name="serviceScope" bind:value={$serviceDetailsFormData.serviceScope} />
							</div>
					...
					{:else}
						<div class="space-y-3 text-sm">
							...
							<!-- Service Scope -->
							<div class="flex items-center">
								<span class="text-muted-foreground font-medium">Service Scope</span>
								<div class="border-muted-foreground/50 mx-2 h-3 flex-1 overflow-hidden border-b border-dotted"></div>
								<span class="font-medium">
									{formatServiceScope(data.serviceItem.serviceScope)}
								</span>
							</div>
						</div>
					{/if}
				...
				<CardContent>
					{#if editPricingInfo}
						...
						>
							...
							<!-- Minimum Pricing -->
							<div class="space-y-2">
								<label class="text-sm font-medium text-gray-700">Minimum Pricing (USD)</label>
								<Input
									type="number"
					...
					{:else}
						<div class="space-y-3 text-sm">
							...
							<!-- Minimum Pricing -->
							<div class="flex items-center">
								<span class="text-muted-foreground font-medium">Minimum Pricing (USD)</span>
								<div class="border-muted-foreground/50 mx-2 h-3 flex-1 overflow-hidden border-b border-dotted"></div>
								<span>
		...
		<div class="space-y-6">
			...
			<!-- Sales Related Details Card -->
			<Card>
				<CardHeader class="flex flex-row items-center justify-between pb-3">
					<CardTitle class="text-lg">Sales Related Details</CardTitle>
					{#if data.canEdit}
						<Button
							variant="ghost"
							size="sm"
							class={['h-6 w-6 p-0', editSalesRelatedDetails && 'opacity-0!']}
							onclick={startEditingSalesRelatedDetails}
							disabled={editSalesRelatedDetails}
						>
							<Icons.Edit class="h-3 w-3" />
						</Button>
					{/if}
				...
				<CardContent>
					{#if editSalesRelatedDetails}
						<form
							method="POST"
							action="?/updateSalesRelatedDetails"
							use:enhance={() => {
								return async ({ result, update }) => {
									if (result.type === 'success') {
										editSalesRelatedDetails = false;
						...
						>
							...
							<div class="rounded-md bg-primary/10 border border-primary/20 p-3">
								<div class="flex items-start gap-2">
									<Icons.Info class="h-4 w-4 text-primary mt-0 flex-shrink-0" />
									<p class="text-xs text-gray-900">
										<strong>Tip:</strong> Display fields can use the variables <code class="bg-primary/20 px-1 py-0.5 rounded">{'{quantity}'}</code>, <code class="bg-primary/20 px-1 py-0.5 rounded">{'{frequency}'}</code>, and <code class="bg-primary/20 px-1 py-0.5 rounded">{'{per_frequency}'}</code>.
									</p>
								</div>
							</div>
							...
							<!-- Recommended Price -->
							<div class="space-y-2">
								<label class="text-sm font-medium text-gray-700">Recommended Price</label>
								<div class="flex gap-2">
									<Input
										type="number"
										bind:value={$salesRelatedDetailsFormData.recommendedPrice}
										...
										class="flex-1"
									/>
									<Select.Root type="single" bind:value={$salesRelatedDetailsFormData.recommendedPriceCurrency}>
										<Select.Trigger class="w-20">
											{$salesRelatedDetailsFormData.recommendedPriceCurrency || 'USD'}
										</Select.Trigger>
										<Select.Content>
									...
									<input type="hidden" name="recommendedPriceCurrency" bind:value={$salesRelatedDetailsFormData.recommendedPriceCurrency} />
								</div>
							</div>

							<!-- Sellable As -->
							<div class="space-y-2">
								<label class="text-sm font-medium text-gray-700">Sellable As</label>
								<Select.Root type="single" bind:value={$salesRelatedDetailsFormData.proposalMode}>
									<Select.Trigger>
										{$salesRelatedDetailsFormData.proposalMode ?
											proposalModeOptions.find(opt => opt.value === $salesRelatedDetailsFormData.proposalMode)?.label || $salesRelatedDetailsFormData.proposalMode
											: 'Select sellable type...'}
									</Select.Trigger>
									<Select.Content>
										{#each proposalModeOptions as option}
											<Select.Item value={option.value}>{option.label}</Select.Item>
										{/each}
									</Select.Content>
								</Select.Root>
								<input type="hidden" name="proposalMode" bind:value={$salesRelatedDetailsFormData.proposalMode} />
							</div>

							<!-- Proposal Display -->
							<div class="space-y-2">
								<label class="text-sm font-medium text-gray-700">Proposal Display</label>
								<Input
									bind:value={$salesRelatedDetailsFormData.packageDisplay}
									name="packageDisplay"
									placeholder="Enter text for how this appears in proposals..."
									class="w-full"
								/>
							</div>
							...
							<!-- MCP Display -->
							<div class="space-y-2">
								<label class="text-sm font-medium text-gray-700">MCP Display</label>
								<Input
									bind:value={$salesRelatedDetailsFormData.mcpDisplay}
									name="mcpDisplay"
									placeholder="Enter text for how this appears in monthly campaign plans..."
									class="w-full"
								/>
							</div>
							...
							<div class="flex justify-end gap-2 pt-2 border-t">
								<Button
									...
									variant="ghost"
									size="sm"
									onclick={cancelEditingSalesRelatedDetails}
								>
									Cancel
								</Button>
								<Button type="submit" size="sm">
					...
					{:else}
						<div class="space-y-3 text-sm">
							<!-- Recommended Price -->
							<div class="flex items-center">
								<span class="text-muted-foreground font-medium">Recommended Price</span>
								<div class="border-muted-foreground/50 mx-2 h-3 flex-1 overflow-hidden border-b border-dotted"></div>
								<span class="font-medium">
									{#if data.serviceItem.recommendedPriceCents}
										{formatCurrency(data.serviceItem.recommendedPriceCents, data.serviceItem.recommendedPriceCurrency || 'USD')}
									{:else}
										Not set
									{/if}
								</span>
							...
							<!-- Sellable As -->
							<div class="flex items-center">
								<span class="text-muted-foreground font-medium">Sellable As</span>
								<div class="border-muted-foreground/50 mx-2 h-3 flex-1 overflow-hidden border-b border-dotted"></div>
								<span class="font-medium">
									{formatProposalMode(data.serviceItem.proposalMode)}
								</span>
							</div>

							<!-- Proposal Display -->
							<div class="flex items-center">
								<span class="text-muted-foreground font-medium">Proposal Display</span>
								<div class="border-muted-foreground/50 mx-2 h-3 flex-1 overflow-hidden border-b border-dotted"></div>
								<span>
									{data.serviceItem.packageDisplay || 'Not specified'}
								</span>
							</div>

							<!-- MCP Display -->
							<div class="flex items-center">
								<span class="text-muted-foreground font-medium">MCP Display</span>
								<div class="border-muted-foreground/50 mx-2 h-3 flex-1 overflow-hidden border-b border-dotted"></div>
								<span>
									{data.serviceItem.mcpDisplay || 'Not specified'}
								</span>
							</div>
						</div>
					{/if}
...
<Dialog.Root bind:open={deleteDialogOpen}>
	<Dialog.Content class="sm:max-w-md">
		<Dialog.Header>
			<Dialog.Title>Deactivate Scope Item</Dialog.Title>
			<Dialog.Description>
				Are you sure you want to deactivate "{data.serviceItem.name}"? This will hide it from the active scope items list.
			</Dialog.Description>
		</Dialog.Header>
