...
const updateServiceDetailsSchema = z.object({
	...
	partnerEntityId: z.preprocess((val) => val === '' || val === null ? null : Number(val), z.number().nullable().optional())
});
...
const updateDeliveryProcessSchema = z.object({
	...
	inStream: z.preprocess((val) => val === 'on' || val === true || val === 'true', z.boolean())
});
...
export const load: PageServerLoad = async (event) => {
	...
	// Get service item with optional partner, activities, employees, all partners, and service categories in parallel
	const [serviceItem, activities, employees, allPartners, allServiceCategories] = await Promise.all([
		event.locals.db
			.select({
				id: serviceItems.id,
				...
				inStream: serviceItems.inStream,
...
export const actions: Actions = {
	...
	updateTimeEstimate: async (event) => {
		if (!event.locals.permissions.has('portal:employee:edit')) {
			return fail(403, { error: 'Forbidden' });
		}
	...
	updateServiceDetails: async (event) => {
		if (!event.locals.permissions.has('portal:employee:edit')) {
			return fail(403, { error: 'Forbidden' });
		}
		...
		try {
			const formData = await event.request.formData();
			const rawData = Object.fromEntries(formData);
			const { name, serviceCategoryId, serviceLabel, serviceScope, partnerEntityId } = updateServiceDetailsSchema.parse(rawData);
			...
			if (oldItem.partnerEntityId !== partnerEntityId) changes.push(`Partner ID: "${oldItem.partnerEntityId || 'None'}" → "${partnerEntityId || 'None'}"`);
	...
	updateDeliveryProcess: async (event) => {
		if (!event.locals.permissions.has('portal:employee:edit')) {
			return fail(403, { error: 'Forbidden' });
		}
		const itemId = parseInt(event.params.id);
		...
		try {
			const formData = await event.request.formData();
			const rawData = Object.fromEntries(formData);
			const { showWork, workProofType, workProofExample, sopUrl, inStream } = updateDeliveryProcessSchema.parse(rawData);
			...
			if (oldItem.inStream !== inStream) changes.push(`In Stream: "${oldItem.inStream ? 'Yes' : 'No'}" → "${inStream ? 'Yes' : 'No'}"`);
	...
	updateSalesRelatedDetails: async (event) => {
		if (!event.locals.permissions.has('portal:employee:edit')) {
			return fail(403, { error: 'Forbidden' });
		}
		const itemId = parseInt(event.params.id);
	...
	deleteItem: async (event) => {
		...
		try {
			...
			await createActivity(
				...
				RELATED_TABLES.service_items,
				itemId,
				`Service item "${item[0].name}" deactivated`
			);

			return { success: true };
		} catch (error) {
			console.error('Error deactivating service item:', error);
			return fail(500, { error: 'Failed to deactivate service item' });
		}
	},

	reactivateItem: async (event) => {
		if (!event.locals.permissions.has('portal:employee:edit')) {
			return fail(403, { error: 'Forbidden' });
		}
		...
		try {
			...
			await createActivity(
				...
				RELATED_TABLES.service_items,
				itemId,
				`Service item "${item[0].name}" reactivated`
			);
